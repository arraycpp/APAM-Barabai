...
if($action == "infodetail") {
  $results = array(); 
  $id_info = trim($_REQUEST['id_info']);
  $sql = "SELECT id_info,judul_info,isi_info,admin,sumber_info, DATE_FORMAT(tgl_info, '%d/%m/%Y') as tanggal FROM appinfo where id_info='$id_info'";
  $result = query($sql);
  while ($row = fetch_array($result)) {
    $results[] = $row;
  }
...
// slide gambar, tapi agak berat jadi belum saya pakai
if($action == "slide") {
  $results = array();
  $sql = "SELECT * FROM appslide order by id_slide desc";
  $result = query($sql);
  while ($row = fetch_array($result)) {
    $results[] = $row;
  }
...
//menampilkan antrian loket / umum
if($action == "umum") {
  $results = array();
  $sql = "SELECT antrian as antrianumum, loket as loketumum FROM antriloket";
  $result = query($sql);
  while ($row = fetch_array($result)) {
    $results[] = $row;
  }
...
//menampilkan antrian bpjs / cs 
if($action == "bpjs") {
  $results = array();
  $sql = "SELECT antrian as antrianbpjs, loket as loketbpjs FROM antrics";
  $result = query($sql);
  while ($row = fetch_array($result)) {
    $results[] = $row;
  }
 ...
 //menampilkan antrian prioritas
if($action == "prioritas") {
  $results = array();
  $sql = "SELECT antrian,loket FROM antriprioritas";
  $result = query($sql);
  while ($row = fetch_array($result)) {
    $results[] = $row;
  }
